...
function Start(fun, ss, tt, rn, counterbalance, posit_ix_, tr_count, isPosterior_ = false, isFirstTrial = true) {
    trial_num = tr_count;
    posit_ix = posit_ix_;
    isPosterior = isPosterior_;
    //Create the stage
    if (!isPosterior) {
        if (isFirstTrial) {
            stage = new Stage("c");
...
function NextTest() {
    ...
    if (test_count < start_state.length) {
        ...
        phase = 1;
        console.log(parent);
        parent.document.getElementById('query2').inngerHTML = parent.prompt_phase1
...
function TestDevice(e) {
    ...
    if (check == true) {
        ...
        buddah_nature = rule(trialdata[trialdata.length - 1]);
        //Add the decision
...
function DrawHistory(td, bn, phase) {
    ...
    if (phase !== 3) {
        ...
        if (isPosterior) {
            let wasSelected = selectedPrior[t - 8];
            console.log(t, 'zendo', '1224');
            console.log(selectedPrior, 'zendo', '1224');
            if (wasSelected) {
